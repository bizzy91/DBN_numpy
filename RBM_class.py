...
class RBM:
    ...
    def CD(self, n):
        ...
        for epoch in range(self.epochs):
            ...
            if epoch%100 == 0:
                print("epoch : ", epoch, np.mean((self.v-self.v1)*(self.v-self.v1)))
    
    def Propagation(self, input_data, W, B):
        v = input_data
        h = self.Sigmoid(np.dot(v, W) + B)
        return h
    
    def Back_Propagation(self, input_data, W, A):
        h = input_data
        v = self.Sigmoid(np.dot(h, W.T) + A)
        return v
